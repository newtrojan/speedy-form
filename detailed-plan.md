...
| Condition                     | Review Type       | Flag Set                       | SLA           |
| ----------------------------- | ----------------- | ------------------------------ | ------------- |
| Distance 31-60 miles          | Distance Review   | (manual)                       | 15 min        |
| Distance 60+ miles            | Manual Quote      | (manual)                       | Call required |
| Vehicle: RV / Motorhome / Bus | Complexity Review | `needs_manual_review`          | 1 hour        |
| No NAGS part match            | Part Lookup       | `needs_manual_review`          | 1 hour        |
| Multiple parts returned       | Part Selection    | `needs_part_selection`         | 15 min        |
| Calibration unknown (NHTSA)   | Tech Review       | `needs_calibration_review`     | 30 min        |
| Customer requests callback    | Callback Queue    | (manual)                       | 15 min        |

### Calibration Review Logic

> **⚠️ Important Distinction:**
>
> - **AUTOBOLT returns `calibration_type=None`** → This is a **definitive answer** meaning the vehicle does NOT require ADAS calibration (e.g., older vehicles without cameras/sensors). This does NOT trigger CSR review.
>
> - **NHTSA+NAGS fallback path (no calibration data)** → This means calibration status is **UNKNOWN** because NHTSA and NAGS don't provide calibration information. This DOES trigger `needs_calibration_review=True` and requires CSR to determine if calibration is needed.
>
> The `VehicleLookupService` correctly handles this by only setting `needs_calibration_review=True` when using the NHTSA+NAGS fallback path.
